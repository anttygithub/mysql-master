...
if [ -n "${REPLICATION_MASTER}" ]; then
    ...
    if [ ! -f /var/lib/mysqlRepSet/replication_set.1 ]; then
        ...
        touch /var/lib/mysqlRepSet/replication_set.1
    else
...
if [ -n "${REPLICATION_MASTER}" ]; then
    ...
    if [ ! -f /var/lib/mysqlRepSet/replication_set.2 ]; then
        ...
        echo "=> Done!"
        touch /var/lib/mysqlRepSet/replication_set.2
    else
        echo "=> MySQL replication master already configured, skip"
    fi
fi
