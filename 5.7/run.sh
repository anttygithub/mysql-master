...
if [ -n "${REPLICATION_MASTER}" ]; then
    ...
    if [ ! -f /var/lib/mysql/replication_set/replication_set.1 ]; then
        ...
        touch /var/lib/mysql/replication_set/replication_set.1
    else
...
if [ -n "${REPLICATION_MASTER}" ]; then
    ...
    if [ ! -f /var/lib/mysql/replication_set/replication_set.2 ]; then
        ...
        echo "=> Done!"
        touch /var/lib/mysql/replication_set/replication_set.2
    else
        echo "=> MySQL replication master already configured, skip"
    fi
fi
